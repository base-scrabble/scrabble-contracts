...
bytes32 private constant _AUTH_TYPEHASH = keccak256("Auth(address player)");
   ...
   // ───────────────
    ...
    event GameCreated(uint256 indexed gameId, address indexed player, uint256 stake);
     ...
     */
    constructor(address wallet, address _superAdmin, address submitter, address backendSigner)
        EIP712("Scrabble", "1")
     ...
     */
    function createGame(uint256 stakeAmount, bytes calldata backendSig) external nonReentrant onlyAuthenticated(backendSig) notBlacklisted whenNotPaused returns (uint256 gameId) {
        // CHECKS
     ...
     * @param backendSig EIP-712 signature from backend authorizing msg.sender.
     */
    function cancelGame(uint256 gameId, bytes calldata backendSig) external onlyAuthenticated(backendSig) notBlacklisted whenNotPaused nonReentrant {
     ...
     * @param backendSig EIP-712 signature from backend authorizing msg.sender.
     */
    function joinGame(uint256 gameId, uint256 stakeAmount, bytes calldata backendSig) external nonReentrant notBlacklisted whenNotPaused onlyAuthenticated(backendSig) {
    ...
    Game storage game = s_games[gameId];
    uint playerCount = game.players.length;
